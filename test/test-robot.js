...
import RobotHistory from '../src/action/RobotHistory';
import RobotReporter from '../src/action/RobotReporter';
import RobotPlacer from '../src/action/RobotPlacer';
import RobotRotater from '../src/action/RobotRotater';

describe('RobotClass Test', function () {
  describe('General', function () {
    const robot = robotFactory();

    it('Able to see default safe mode as on', function () {
      expect(robot.getSafeMode()).to.be.true;
    });

    it('Able to turn off the safe mode', function () {
      robot.setSafeMode(false);
      expect(robot.getSafeMode()).to.be.false;
    });

    it('Able to set dimensions', function () {
      robot.setDimensions();
  ...
  describe('Main', function () {
    ...
    it('Able to get the default position', function () {
      expect(robot.getPosition()).to.deep.equal({ x: 0, y: 0 });
    });

    it('Able to get the default direction', function () {
      expect(robot.getDirection()).to.equal(RobotDirection.NORTH);
    });

    it('report() - Able to report the default position and direction in the required output format', function () {
    ...
    it('place() - Able to place at a specific coordinate', function () {
    ...
    it('move() - given it is now at the position of 0,0 and facing NORTH, Able to move 1 step up', function () {
    ...
    it('rotate() - given it is now facing NORTH, Able to rotate to the right to face EAST', function () {
      const initialPlacement = '0,0,NORTH';
    ...
    it('rotate() - given it is now facing NORTH, Able to rotate twice to the right to face SOUTH', function () {
      const initialPlacement = '0,0,NORTH';
